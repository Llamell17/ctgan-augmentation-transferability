...
from src.ctgan_utils import seed_everything, fit_and_sample_ctgan
...
def run_experiment(df_train, df_test, df_hybrid_train, seed):
    ...
    # SHAP using test data
    best_da = daxgb.best_estimator_
    explainer = shap.TreeExplainer(best_da)
    sv_test = explainer(X_test)
    
    shap.plots.beeswarm(sv_test, max_display=10)
    shap.plots.bar(sv_test, max_display=10)

    vals = sv_test.values
    mean_abs = np.abs(vals).mean(axis=0)
    shap_rank = pd.DataFrame({"feature": feature_cols, "mean_abs_shap": mean_abs}) \
    ...
    return df_kpi, best_params, preds, shap_rank

# Crossvalidation
from src.ctgan_utils import seed_everything

def cv5(df_train, best_params, best_params_model_c0, best_params_model_c1, n0_sample, n1_sample, seed):
    feature_cols = ["UCS","RD","TC","W","E"]
    target_col   = "OB"
    ...
    for tr_idx, va_idx in kf.split(df_train):
        ...
        n0_sample_cv = int(round(n0_sample / 5))
        n1_sample_cv = int(round(n1_sample / 5))
        target_new = {
            0: (n0_sample_cv, best_params_model_c0), 
            1: (n1_sample_cv, best_params_model_c1)
        }  
           
        synth_parts = []
        for c, (n_new, params_c) in target_new.items():
            df_c = data_tr.loc[data_tr["cluster"] == c, feature_cols + [target_col]].copy()
            seed_everything(seed)
            syn_c = fit_and_sample_ctgan(
                        df_in = df_c,
                        n_sample = n_new,
                        seed = seed,
                        chart=False,
